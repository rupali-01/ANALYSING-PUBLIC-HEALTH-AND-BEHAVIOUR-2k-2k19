...
library('dplyr')      # For data manipulation (filtering, selecting, mutating, summarizing)
library('readxl')     # For reading Excel files (.xls and .xlsx formats)
library('tidyr')      # For tidying and reshaping messy datasets
library('janitor')    # For data cleaning (e.g., cleaning column names, making tables)
library('skimr')      # For summarizing and providing a quick overview of data
library('sqldf')      # For executing SQL queries directly on R data frames
library('plotrix')    # For specialized plotting functions (3D pie charts, radial plots, etc.)
library('knitr')      # For generating dynamic reports (tables, documents, and markdown files)
library('reshape2')   # For melting and casting data frames (data reshaping)
...
setwd("C:/Users/rupali shrivastava/OneDrive/Desktop/R-PROJECT")
